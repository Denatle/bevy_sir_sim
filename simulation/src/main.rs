    ...
    App::new()
        ...
        .add_systems(
            ...
            (
                ...
                change_destination,
                move_agents,
                (type_agents, color_agents).chain(),
            ),
        )
        .run();
}
...
fn move_cursor(
    ...
    for (agent, mut transform) in squares.iter_mut() {
        ...
        transform.translation.x = coords
            .x
            .clamp(0. - simul.canvas_w / 2., simul.canvas_w / 2.);
        transform.translation.y = coords
            .y
            .clamp(0. - simul.canvas_h / 2., simul.canvas_h / 2.);
    }
}
...
fn change_destination(
    ...
    for (mut agent, chunkable, transform) in agent_query.iter_mut() {
        ...
        if agent.destination.round() == transform.translation.round() {
            let x_dev =
                chunkable.coords.x as f32 + ((random::<f32>() - 0.5) * 2.).round_ties_even();
            let y_dev =
                chunkable.coords.y as f32 + ((random::<f32>() - 0.5) * 2.).round_ties_even();
...
fn type_agents(
    ...
    for entity in chunk_entities {
        ...
        let (mut agent, _chunkable) = agent_query.get_mut(entity).unwrap();
